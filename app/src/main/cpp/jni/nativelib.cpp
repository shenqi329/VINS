...
//    LOGI("buf.stride: %d", buf.stride);
    ...
    Mat dstRgba;
    if(isScreenRotated) {
        dstRgba = Mat(width, buf.stride, CV_8UC4, dstPtr); // TextureView buffer, use stride as width
    } else {
        dstRgba = Mat(height, buf.stride, CV_8UC4, dstPtr); // TextureView buffer, use stride as width
    }
    ...
    viewControllerGlobal->processImage(rotatedRgba, timeStampSec, false);
