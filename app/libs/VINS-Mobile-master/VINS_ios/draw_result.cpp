...
void DrawResult::drawGround(cv::Mat &result, vector<Vector3f> &point_cloud, Vector3f P_latest, Matrix3f R_latest)
{
    ...
    for(auto e = begin(edges); e != end(edges); e++) {
        ...
        pts2.x=(*e).p2.x;
        pts2.y=(*e).p2.y;
        cv::line(result, pts, pts2, Scalar(100,100,100), 1, 8, 0);
    }
...
void DrawResult::drawAR(cv::Mat &result, vector<Vector3f> &point_cloud, Vector3f P_latest, Matrix3f R_latest)
{
    ...
    if (longPressFlag)
    {
        ...
        if ( dis_min < 120 and Grounds.size()>0 )
        {
            ...
            cv::circle(result, pts1, 0, Scalar(0,255,0), 12);
        }
        longPressFlag = false;
    }
    //////////////////////////// long press unlock response
...
void DrawResult::Reprojection(cv::Mat &result, vector<Vector3f> &point_cloud, const Matrix3f *R_window,const Vector3f *T_window, bool box_in_trajectorty)
{
    ...
    //draw frame arrow
    {
        ...
        arrowedLine(result, pt1, pt2, Scalar(100,100,100),1, 8, 0, 0.02);
        cv::putText(result, "X", pt2, 0, 0.5, Scalar(100,100,100));
        ...
        arrowedLine(result, pt1, pt2, Scalar(100,100,100),1 , 8, 0, 0.02);
        cv::putText(result, "Y", pt2, 0, 0.5, Scalar(100,100,100));
        ...
        arrowedLine(result, pt1, pt2, Scalar(100,100,100), 1, 8, 0, 0.02);
        cv::putText(result, "Z", pt2, 0, 0.5, Scalar(100,100,100));
        ...
        //draw grid
        {
            ...
            for(auto it : grid_space)
            {
                ...
                cv::line(result, pts, pts2, Scalar(180,180,180), 1, 8, 0);
            }
        }
    }
    //draw camera
    for(int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        Scalar camera_color = Scalar(0,0,255);
    ...
    for (int i=0; i<point_cloud.size(); i++)
    {
        ...
        if(checkBorder(pts))
        {
            cv::circle(result, pts, 0, Scalar(0,255,0), 3);
        }
    }
