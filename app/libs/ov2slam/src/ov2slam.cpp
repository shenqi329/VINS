...
double time_now_sec()
{
    long time_now = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch()).count();

    double ms = time_now / 1000.f;
...
void SlamManager::run()
{
    ...
    while( !bexit_required_ ) {
        ...
        if( getNewImage(img_left, img_right, time) )
        {
            ...
            if( frame_id_ > 0 ) {
                cam_delay = time_now_sec() - last_img_time;
                last_img_time += cam_delay;
            } else {
                last_img_time = time_now_sec();
            }
            ...
            // ========================================
            bool c1 = cam_delay > 0;
            bool c2 = time_now_sec() > 100. * cam_delay;
